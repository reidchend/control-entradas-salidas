...
def _extract_from_image_ocrspace(image_path: str) -> str:
    ...
    try:
        response = requests.post('https://api.ocr.space/parse/image',
            data={'base64Image': f"data:image/png;base64,{img_data}",'language': 'spa','isOverlayRequired': 'false','detectOrientation': 'true','scale': 'true','OCREngine': '2'},
            headers=headers, timeout=30)
        if not response.content: return None
        result = response.json()
        if result.get("IsErroredOnProcessing"): return None
        parsed_results = result.get("ParsedResults", [])
        if not parsed_results: return None
        full_text = "\n".join(r.get("ParsedText", "") for r in parsed_results)
        return full_text.strip() if full_text.strip() else None
    except Exception: return None

def parse_factura_text(text: str) -> dict:
    data = {"proveedor": "", "rif": "", "nro_factura": "", "fecha": "", "tipo_documento": "Factura"}
    text = text.strip()
    prov_section = text
    for line in text.split('\n'):
        if re.search(r'\b(Proveedor|Emitido por|Vendido por|Nombre o Razón Social)\b', line, re.IGNORECASE):
    ...
    if nro_match:
        ...
        if "NOTA" in nro_match.group(0).upper(): data["tipo_documento"] = "Nota de Entrega"
        elif "ENTRADA" in nro_match.group(0).upper(): data["tipo_documento"] = "Entrada"
        elif "FACTURA" in nro_match.group(0).upper(): data["tipo_documento"] = "Factura"
    else:
        for line in text.split('\n'):
            line = line.strip()
            if re.fullmatch(r'\d{6,10}', line):
                data["nro_factura"] = line
                break
...
def extract_from_image(image_path: str, use_preprocessing: bool = True) -> dict:
    if not image_path: return {"proveedor": "", "rif": "", "nro_factura": "", "fecha": ""}
    raw = _extract_from_image_ocrspace(image_path)
    if raw: return parse_factura_text(raw)
    full_text = ""
    try:
        import pytesseract
        from PIL import Image
...
def preprocess_image(image_path: str) -> str:
    try:
        import cv2
        if not os.path.exists(image_path): return image_path
